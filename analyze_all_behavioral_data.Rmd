...
cat("\n✓ Loaded", sum(sapply(ld_all_data, length)), "LD trials\n")
if (length(ld_errors) > 0) {
  cat("✗ Errors in", length(ld_errors), "trials\n")
}
```
...
for (session_name in names(ld_all_data)) {
  ...
  for (trial_name in names(session_data)) {
    ...
    # Analyze each arena
    for (arena_name in names(trial_data)) {
      arena_data <- trial_data[[arena_name]]

      tryCatch({
        results <- analyze_ld(arena_data$data, fps = 25)

        # Store results with full identifier
        result_id <- paste(session_name, trial_name, arena_name, sep = "_")
        ...
        cat("Error analyzing", arena_name, ":", e$message, "\n")
      })
    }
  }
}
...
cat("✓ Analyzed", nrow(ld_results_df), "LD arenas\n")
```
...
if (nrow(ld_results_df) > 0) {
  ...
  p1 <- ggplot(ld_results_df, aes(x = arena)) +
    ...
    labs(title = "LD: Time in Light vs Dark Zones",
         x = "Arena", y = "Time (seconds)", fill = "Zone") +
    theme_minimal() +
...
if (nrow(ld_results_df) > 0) {
  ...
  cat("✓ Exported LD results to:", ld_output_file, "\n")
}
```
...
for (session_dir in oft_sessions) {
  ...
  for (file in excel_files) {
    ...
    tryCatch({
      data <- load_oft_data(file, fps = 25)
...
cat("\n✓ Loaded", sum(sapply(oft_all_data, length)), "OFT trials\n")
if (length(oft_errors) > 0) {
  cat("✗ Errors in", length(oft_errors), "trials\n")
}
```
...
for (session_name in names(oft_all_data)) {
  ...
  for (trial_name in names(session_data)) {
    ...
    for (arena_name in names(trial_data)) {
      ...
      tryCatch({
        results <- analyze_oft(arena_data$data, fps = 25)
...
cat("✓ Analyzed", nrow(oft_results_df), "OFT arenas\n")
```
...
if (nrow(oft_results_df) > 0) {
  ...
  p1 <- ggplot(oft_results_df, aes(x = arena, y = as.numeric(pct_time_in_center), fill = session)) +
...
if (nrow(oft_results_df) > 0) {
  ...
  cat("✓ Exported OFT results to:", oft_output_file, "\n")
}
```
...
for (session_dir in nort_sessions) {
  ...
  for (file in excel_files) {
    ...
    tryCatch({
      # Note: NORT requires novel_side specification
      # Using "left" as default - adjust per experiment
      data <- load_nort_data(file, fps = 25, novel_side = "left")
...
cat("\n✓ Loaded", sum(sapply(nort_all_data, length)), "NORT trials\n")
if (length(nort_errors) > 0) {
  cat("✗ Errors in", length(nort_errors), "trials\n")
}
```
...
for (session_name in names(nort_all_data)) {
  ...
  for (trial_name in names(session_data)) {
    ...
    for (arena_name in names(trial_data)) {
      ...
      tryCatch({
        results <- analyze_nort(arena_data$data, fps = 25)
...
cat("✓ Analyzed", nrow(nort_results_df), "NORT arenas\n")
```
...
if (nrow(nort_results_df) > 0) {
  nort_summary <- nort_results_df %>%
    summarise(
      ...
      mean_novel_time = mean(as.numeric(exploration_novel_sec), na.rm = TRUE),
      mean_familiar_time = mean(as.numeric(exploration_familiar_sec), na.rm = TRUE),
      pct_valid_trials = sum(as.logical(trial_valid), na.rm = TRUE) / n() * 100
    )
...
if (nrow(nort_results_df) > 0) {
  ...
  nort_long <- nort_results_df %>%
    select(arena, session, exploration_novel_sec, exploration_familiar_sec) %>%
    pivot_longer(cols = c(exploration_novel_sec, exploration_familiar_sec),
...
if (nrow(nort_results_df) > 0) {
  ...
  cat("✓ Exported NORT results to:", nort_output_file, "\n")
}
```
...
for (session_dir in epm_sessions) {
  ...
  for (file in csv_files) {
    ...
    tryCatch({
      ...
      data <- load_epm_data(file, fps = 25, pixels_per_cm = 5.3,
...
cat("\n✓ Loaded", sum(sapply(epm_all_data, length)), "EPM subjects\n")
if (length(epm_errors) > 0) {
  cat("✗ Errors in", length(epm_errors), "subjects\n")
}
```
...
cat("✓ Analyzed", nrow(epm_results_df), "EPM subjects\n")
```
...
if (nrow(epm_results_df) > 0) {
  ...
  cat("✓ Exported EPM results to:", epm_output_file, "\n")
}
```
...
if (length(epm_all_data) > 0) {
  ...
  cat("✓ Generated EPM plots in:", epm_plots_dir, "\n")
}
```
  ...
  for (session_name in names(ld_all_data)) {
    ...
    for (trial_name in names(session_data)) {
      ...
      for (arena_name in names(trial_data)) {
        ...
        tryCatch({
          subject_id <- paste(session_name, trial_name, arena_name, sep = "_")
          subject_dir <- file.path(ld_plots_dir, session_name, trial_name, arena_name)
          generate_ld_report(arena_data, output_dir = subject_dir, subject_id = subject_id)

        }, error = function(e) {
          cat("Error generating plots for", arena_name, ":", e$message, "\n")
        })
      }
    }
  }

  cat("✓ Generated LD plots in:", ld_plots_dir, "\n")
}
```
...
if (exists("ld_results_df")) cat("  ✓ LD:", nrow(ld_results_df), "arenas\n")
if (exists("oft_results_df")) cat("  ✓ OFT:", nrow(oft_results_df), "arenas\n")
if (exists("nort_results_df")) cat("  ✓ NORT:", nrow(nort_results_df), "arenas\n")
if (exists("epm_results_df")) cat("  ✓ EPM:", nrow(epm_results_df), "subjects\n")

cat("\nOutput Files:\n")
cat("  📊 Combined results CSV files in:", output_dir, "\n")
cat("  📈 Individual plots in:", output_dir, "/{paradigm}_individual_plots/\n")
cat("  📄 This HTML report\n")
