...
if (!is.null(params$occupancy)) {
  ...
  if ("time_seconds" %in% names(occupancy_display)) {
    occupancy_display$time_seconds <- round(occupancy_display$time_seconds, 2)
  }
  ...
  display_cols <- c("zone_id", "n_frames", "time_seconds", "percentage")
  if (all(display_cols %in% names(occupancy_display))) {
    occupancy_display <- occupancy_display[, display_cols]
    kable(occupancy_display,
          caption = "Time spent in each zone",
          col.names = c("Zone", "Frames", "Time (s)", "Percentage (%)"))
  } else {
    # Fallback: display all columns without custom names
    kable(occupancy_display,
          caption = "Time spent in each zone")
  }
} else {
...
if (!is.null(params$entries)) {
  ...
  # Select only relevant columns for display
  display_cols <- c("zone_id", "n_entries", "mean_duration", "total_time")
  if (all(display_cols %in% names(entries_display))) {
    entries_display <- entries_display[, display_cols]
    kable(entries_display,
          caption = "Zone entry statistics",
          col.names = c("Zone", "Number of Entries", "Mean Duration (s)", "Total Time (s)"))
  } else {
    # Fallback: display all columns without custom names
    kable(entries_display,
          caption = "Zone entry statistics")
...
if (!is.null(params$latency)) {
  ...
  # Format numeric columns
  if ("latency_seconds" %in% names(latency_display)) {
    latency_display$latency_seconds <- round(latency_display$latency_seconds, 2)
  }

  # Select only relevant columns for display
  display_cols <- c("zone_id", "latency_seconds", "first_entry_frame")
  if (all(display_cols %in% names(latency_display))) {
    latency_display <- latency_display[, display_cols]
